...
/*   Updated: 2023/07/27 12:16:16 by valentin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
std::string  &Server::get_rcv_fd(int fd)
{
    for (std::map<int, std::string>::iterator it = this->rcv_client.begin(); it != this->rcv_client.end(); ++it)
    {
        if (it->first == fd)
        {
            return (it->second);
...
void  Server::add_rcv(int fd, std::string str)
{
    try {
